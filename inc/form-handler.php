...
{
    $phpmailer->isSMTP();
    $phpmailer->Host       = 'seoservicecanada.com';
    $phpmailer->SMTPAuth   = true;
    $phpmailer->Port       = 465;
    $phpmailer->Username   = 'info@seoservicecanada.com';
    $phpmailer->Password   = 'mlL06dfj){z#@8]%';
    $phpmailer->SMTPSecure = 'ssl';
    $phpmailer->From       = 'info@seoservicecanada.com';
    $phpmailer->FromName   = 'SEO Service Canada - Leads';

    // Force the sender address (prevents WordPress from overriding).
    $phpmailer->Sender = 'info@seoservicecanada.com';
    $phpmailer->clearReplyTos();
    $phpmailer->addReplyTo('info@seoservicecanada.com', 'SEO Service Canada');
}
add_action('phpmailer_init', 'ssc_configure_smtp');

/**
 * Force WordPress wp_mail() to use our verified From address.
 * This prevents the default "wordpress@yourdomain.com" override
 * that can cause SPF / "domain not allowed in From" errors.
 */
add_filter('wp_mail_from', function ($email) {
    return 'info@seoservicecanada.com';
}, 999);

add_filter('wp_mail_from_name', function ($name) {
    return 'SEO Service Canada - Leads';
}, 999);
...
function ssc_handle_audit_form()
{
    ...
    $services   = isset($_POST['audit_services']) ? sanitize_text_field($_POST['audit_services']) : 'Not selected';
    $message    = isset($_POST['audit_message']) ? sanitize_textarea_field($_POST['audit_message']) : '';
    ...
    $esc_services   = esc_html($services);
    $esc_message    = !empty($message) ? nl2br(esc_html($message)) : '';
    $esc_page_title = esc_html($page_title);
    $esc_page_url   = esc_url($page_url);
    $submission_date = date('F j, Y \a\t g:i A');

    // Services row — only show if any selected
    $services_row = '';
    if (!empty($services) && $services !== 'Not selected') {
        $services_row = "<div class='lead-row'>
            <span class='lead-icon'>&#9989;</span>
            <span class='lead-details'>
                <span class='lead-label'>Services Interested In</span>
                <span class='lead-value'>{$esc_services}</span>
            </span>
        </div>";
    }

    // Message block — only show if provided
    $message_block = '';
    if (!empty($esc_message)) {
        $message_block = "<div style='margin-top: 30px;'>
            <span style='color:#bf2021;font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:2.5px;margin:0 0 15px 0;display:block;'>Client's Message</span>
            <div style='background:#ffffff;padding:28px;border-radius:16px;border-left:6px solid #bf2021;color:#334155;font-size:15px;line-height:1.8;font-style:italic;box-shadow:0 8px 25px rgba(0,0,0,0.04);'>
                {$esc_message}
            </div>
        </div>";
    }
...
/**
 * Enqueue contact form script and localize AJAX data.
 */
function ssc_enqueue_contact_form_script()
{
    wp_enqueue_script(
        'ssc-contact-form',
        SSC_THEME_URI . '/assets/js/contact-form.js',
        array(),
        SSC_THEME_VERSION,
        array('in_footer' => true, 'strategy' => 'defer')
    );

    wp_localize_script('ssc-contact-form', 'sscContactForm', array(
        'ajaxurl' => admin_url('admin-ajax.php'),
        'nonce'   => wp_create_nonce('ssc_contact_form_nonce'),
    ));
}
add_action('wp_enqueue_scripts', 'ssc_enqueue_contact_form_script');

/**
 * AJAX Handler for Contact Page Form
 */
function ssc_handle_contact_form()
{
    // Security Check
    check_ajax_referer('ssc_contact_form_nonce', 'nonce');

    // Sanitize Form Data
    $name       = sanitize_text_field($_POST['ssc_name']);
    $email      = sanitize_email($_POST['ssc_email']);
    $phone      = isset($_POST['ssc_phone']) ? sanitize_text_field($_POST['ssc_phone']) : 'Not provided';
    $website    = isset($_POST['ssc_website']) ? esc_url_raw($_POST['ssc_website']) : 'Not provided';
    $service    = isset($_POST['ssc_service']) ? sanitize_text_field($_POST['ssc_service']) : 'Not selected';
    $message    = sanitize_textarea_field($_POST['ssc_message']);
    $page_url   = isset($_POST['page_url']) ? esc_url($_POST['page_url']) : 'Unknown';
    $page_title = isset($_POST['page_title']) ? sanitize_text_field($_POST['page_title']) : 'Unknown Page';
    ...
    $subject = 'New Contact Inquiry: ' . $name . ' — ' . $service_display;
    ...
    $esc_phone       = esc_html($phone);
    $esc_website     = esc_html($website);
    $esc_service     = esc_html($service_display);
    $esc_message     = nl2br(esc_html($message));
    $esc_page_title  = esc_html($page_title);
    $esc_page_url    = esc_url($page_url);
    $submission_date = date('F j, Y \a\t g:i A');

    // Website row — only show if provided
    $website_row = '';
    if (!empty($website) && $website !== 'Not provided') {
        $website_row = "<div class='lead-row'>
            <span class='lead-icon'>&#127760;</span>
            <span class='lead-details'>
                <span class='lead-label'>Website URL</span>
                <span class='lead-value'><a href='{$esc_website}' style='color:#302e9d;text-decoration:none;font-weight:700;'>{$esc_website}</a></span>
            </span>
        </div>";
    }

    // Premium HTML Email Template — Contact Form
    $email_body = <<<HTML
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        .hero-badge {
            display: inline-block;
            background: linear-gradient(135deg, rgba(48,46,157,0.08) 0%, rgba(191,32,33,0.08) 100%);
            color: #302e9d;
            padding: 8px 20px;
            border-radius: 50px;
            font-size: 11px;
            ...
            letter-spacing: 2.5px;
            margin-bottom: 18px;
            border: 1px solid rgba(48,46,157,0.12);
        }
        .main-title {
            color: #302e9d;
            font-size: 30px;
...
<body>
    <div class="email-wrapper">
        <div class="email-container">
            ...
            <div class="hero-section">
                <span class="hero-badge">New Contact Inquiry</span>
                <h1 class="main-title">Message Received!</h1>
                <p class="sub-title">Someone reached out via the Contact page. Review their details and respond promptly.</p>
                <span class="submission-time">Submitted on {$submission_date}</span>
            </div>

            <div class="content-body">
                <div class="leads-card">
                    <p class="leads-card-title">Contact Details</p>
                    ...
                    <div class="lead-row">
                        ...
                        <span class="lead-details">
                            <span class="lead-label">Email Address</span>
                            <span class="lead-value"><a href="mailto:{$esc_email}" style="color:#302e9d;text-decoration:none;font-weight:700;">{$esc_email}</a></span>
                        </span>
                    </div>
                    ...
                    <div class="lead-row">
                        <span class="lead-icon">&#128269;</span>
                        <span class="lead-details">
                            <span class="lead-label">Service Interested In</span>
                            <span class="lead-value"><span class="service-badge">{$esc_service}</span></span>
                        </span>
                    </div>
                </div>
...
HTML;
    ...
    if ($sent) {
        wp_send_json_success(array('message' => 'Thank you for reaching out! We have received your message and will get back to you within 24 hours.'));
    } else {
        $error_msg = !empty($ssc_mail_errors) ? implode(' | ', $ssc_mail_errors) : 'Unknown mail error.';
        wp_send_json_error(array('message' => 'Mail Error: ' . $error_msg));
    }
}
add_action('wp_ajax_ssc_handle_contact_form', 'ssc_handle_contact_form');
add_action('wp_ajax_nopriv_ssc_handle_contact_form', 'ssc_handle_contact_form');
